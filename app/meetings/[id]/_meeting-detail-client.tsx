...
export default function MeetingDetailClient({
  ...
  async function confirmAdd() {
    ...
    await Promise.all(toAdd.map((m) =>
      fetch(`/api/meetings/${meetingId}/attendees`, {
        method: "POST",
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_240px] gap-12 items-start">
        ...
        <div className="order-2 md:order-1 pb-20 md:pb-0">
          <div className="flex items-end justify-between mb-2">
            ...
            <Link
              href={`/reviews/new?meetingId=${meetingId}`}
          ...
          {reviews.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <Link href={`/reviews/new?meetingId=${meetingId}`} className="text-xs text-[#8B3A2A] hover:underline underline-offset-2">
                첫 번째 독후감 작성하기 →
              </Link>
            </div>
          ) : (
                ...
                {(() => {
                  ...
                  return (
                    <div className="space-y-4">
                      ...
                      <Button
                        onClick={confirmAdd}
                        disabled={selected.size === 0 || adding || refreshing}
                        className="w-full bg-[#1C1A17] hover:bg-[#8B3A2A] transition-colors cursor-pointer rounded-full"
                      >
                        {adding || refreshing ? "추가 중..." : `${selected.size > 0 ? `${selected.size}명 ` : ""}참석자 추가`}
                      </Button>
                    </div>
                  );
                })()}
